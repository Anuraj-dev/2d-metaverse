...
- **The complete live tracker graph is parent #90 with native sub-issues #91–#132 and 130 native blocking edges.** Every child has acceptance criteria plus independent review, CI, and deployment-verification gates. #91 is merged/closed. Draft PR #135 implements #92 (truthful auth failures), and draft PR #136 implements #93 (deterministic `WorldScene` listener teardown); both still require independent review and green CI. Promote later issues only when their native blockers are closed and the milestone order permits them.
...
## Status
- **Phase B 2026-07-11:** opened draft PR #136 for #93. Every `WorldScene` Net, EventBus, scale, and development-hook listener now has deterministic shutdown/destroy ownership through a pure `ListenerScope`; review follow-up added a public-boundary `WorldScene.create()` → shutdown → remount regression alongside the pure owner tests. All four targeted tests and touched-file ESLint passed. Independent review and CI are pending.
- **Phase B 2026-07-11:** PR #134 for #91 merged as `1f7c368` and the issue is closed. Local and production Compose pin LiveKit v1.9.12, and backend integration plus frontend E2E probe `/rtc/v1/validate` before media checks.
