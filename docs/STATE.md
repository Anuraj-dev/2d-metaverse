...
- **The complete live tracker graph is parent #90 with native sub-issues #91–#132 and 130 native blocking edges.** Every child has acceptance criteria plus independent review, CI, and deployment-verification gates. Only the immediate non-overlapping foundation frontier carries `ready-for-agent`: #91 (LiveKit parity), #92 (truthful auth failures), and #93 (WorldScene lifecycle). Promote later issues only when their native blockers are closed and the milestone order permits them. No implementation PR is open yet; start at most two implementers plus one independent reviewer.
