# 2D Metaverse — State
> Gather/Zep-style 2D metaverse (app name: **hyprverse**): walk a styled office campus, proximity voice/video, admin-gated meeting rooms, arcade + board-game tables. · Last checkpoint: 2026-07-11 (pilot issue graph)
...
- **The complete live tracker graph is parent #90 with child issues #91–#132.** Every child has acceptance criteria, real blocking references, `ready-for-agent`, and independent review/CI/deploy gates. No implementation PR is open yet. Immediate non-overlapping foundation frontier: #91 (LiveKit parity), #92 (truthful auth failures), and #93 (WorldScene lifecycle). Start at most two implementers plus one independent reviewer; later unblocked safety/map/game issues remain sequenced behind the foundation milestone even when their graph has no technical blocker.
...
## Status
- **Phase A 2026-07-11:** merged the owner direction and independently reviewed pilot-delivery spec; published GitHub parent #90 and 42 dependency-ordered child issues (#91–#132). Next implementation priority is #91/#92, with #93 following as reviewer capacity allows.
