...
## 2026-07-11 — Device consent is session-scoped and defaults off
**Why:** A new arrival must never request or publish microphone/camera before an explicit choice. Mic/cam preferences therefore default off and persist only in `sessionStorage`: the choice survives reloads and world/room/stage transitions in the same tab page-session, while an empty page session returns to off (explicitly duplicated tabs may inherit a browser-provided copy). `localStorage` persistence was rejected because it would silently carry yesterday's publish consent into a future visit. Fresh LiveKit connections skip capture APIs entirely for disabled devices; existing serialized room/stage privacy cut-offs remain immediate.
...
## 2026-07-11 — Server geometry manifest: generated, versioned, committed under backend/assets (PRD 25.20)
**Why:** The backend previously mirrored campus geometry by hand in three places (`seed.ts` door zones/seats, `stage.ts` stage/presenter rects, `socket.ts` spawn), with zero coupling to `campus.json` — silent drift risk. `gen_campus.py` now emits one versioned manifest (`backend/assets/campus.geometry.json`) from the same authored data as `campus.json`, covering world bounds, walkability (walls grid), doors, private-room seats, board chairs, stage/presenter zones, and portals. Schema + version live in `@metaverse/shared` (`geometryManifestSchema` / `GEOMETRY_MANIFEST_VERSION`); the backend `safeParse`s it and refuses readiness (and fails startup) on an invalid or version-mismatched (stale) manifest. **Location:** committed next to the backend and copied into the Docker image (`COPY backend/assets`) — the image ships no frontend assets, and the build context is the repo root, so an inside-the-image artifact is the only reliable server-side source. **Manual mirrors kept but cross-checked:** rather than a risky big-bang replacement, `stage.ts`/`seed.ts`/`spawn` stay as-is but `backend/test/geometry.test.ts` asserts each equals the manifest (seats carry a documented tile-centre +½-tile offset), so drift now fails CI. Downstream slices 25.21–25.24 (movement envelope, walkability/collision, door/seat/board proximity, stage hardening) consume this manifest instead of re-deriving geometry. Analytics/security-event emission for this slice is deferred until the analytics foundation (#99) merges.
## 2026-07-11 — Product analytics is server-stamped, allowlisted, and operator-read
**Why:** Pilot measurement must not become a second identity/content collection system. Pre-auth sign-in outcomes therefore use anonymous server-generated attempt UUIDs and coarse results retained for 7 days; authenticated client events derive actor and time on the server, pass a strict shared allowlist, deduplicate the full bounded payload by event UUID, and expire after 90 days. The backend physically prunes expiry on startup and every six hours without relying on traffic, while every operator query independently excludes expired rows. Reads stay behind operator database credentials through reviewed SQL summary/export files; there is no public analytics read API. The foundation allowlist contains only a deployment-verification probe excluded from KPI summaries; each later feature slice must add and emit its own bounded privacy-reviewed schema.

## 2026-07-25 — Arcade 2.0 (spec 27): race-not-duel, space merge theme, plausibility-only anti-cheat, Opus 5 builds everything
**Why:** Raja judged the arcade's retention near-zero (Snake "trash": one too-fast speed, no levels; bare visuals everywhere; static all-time boards). Locked in a grilling session: (1) **Race mode over score-duel** — a duel forces the early-dead player to wait for the survivor, killing flow; a race (identical seed/layout, first-to-goal wins, most-progress on all-dead) keeps everyone playing to the last second, works 1–N players, and solo becomes race-the-clock. (2) **Merge-drop flagship uses an original space theme** (pebble→…→sun) — merge-on-collision mechanics are IP-safe but fruit/Suika theming is not; Tetris-likes rejected outright on litigation history. (3) **Anti-cheat = plausibility checks only** (ceilings, pace vs duration, monotonicity) — Raja chose it over deterministic replay-verification despite the reducers making replay feasible; leaderboard credibility vs work trade-off, revisitable later. (4) **Opus 5 implements all four tickets, UI AND backend** — deliberate override of the "heavy backend → GPT Sol" routing to benchmark Opus 5 end-to-end (`docs/benchmark.md`, orchestrator-owned); review rigor unchanged (Sol high first pass + codex cloud experiment). Spec: GitHub #162; tickets #163–#166.

## 2026-07-30 — Arcade overlay: Escape = pause menu, never auto-fullscreen (three-model consult)
**Why:** Two Esc/fullscreen iterations were rejected by Raja (auto-fullscreen double-Escape, then quit-confirm-on-Escape). Fable 5, Opus 5, and Sol were consulted independently with one brief and unanimously converged: the overlay never takes browser fullscreen on its own (full-bleed CSS shell instead; ⛶ is an explicit opt-in), Escape opens a pause menu (Resume/Restart/Quit — the menu IS the quit confirmation, dialog deleted), and a browser-eaten Escape in opt-in fullscreen lands on the same pause menu with the fullscreen intent restored inside Resume's gesture. Benchmark: Opus's state machine won, Fable's rationale/CSS second, Sol third but strongest as reviewer (caught over-phase resurrection + flappy resize-teleport). All fullscreen calls go through one hardened door (`ui/arcade/fullscreen.ts`) after a sync-throw bug left the button permanently disabled. Alternative rejected: any auto-fullscreen or confirm-dialog variant.
