...
| `board_place.ogg`, `board_place_far.ogg`, `board_win.ogg`, `board_lose.ogg`, `board_draw.ogg`, `board_invalid.ogg` | Mid knock (`2-Loop(Drums Only)` @ 97.33s) + low thump (`6-Drums Only` @ 10.06s) + melodic notes from the `8-Intro` arpeggio | Board tables (`scripts/curate_audio.py` → `curate_board_sounds`), replacing the arcade blips these cues used to borrow. Wooden placement foley at two distances — yours dry and close, your opponent's pitched down, low-passed and roomier — plus three separate outcomes in the same key family as the other chimes: a rising three-note win, a soft darkened two-note loss, and a deliberately flat repeated note for a draw. `board_invalid` is the same thump gated hard and low-passed into a dead rejection. |
| `arcade_start.ogg`, `arcade_point.ogg`, `arcade_over.ogg`, `arcade_flap.ogg`, `arcade_hit.ogg` | **Project-original** — synthesized square-wave chiptune / sine+pink-noise one-shots (`scripts/curate_audio.py` → `synth_arcade`) | Diegetic 8-bit arcade blips (open arpeggio, score blip, game-over descend) plus the flappy port's in-game pair: a soft wingbeat (falling sine + air puff, mixed far quieter since it fires several times a second) and the crash thud (low sine drop + body noise). Intentionally a different family from the recorded foley — a cozy pack has no arcade beeps to cut. Project asset (no third-party source). |
