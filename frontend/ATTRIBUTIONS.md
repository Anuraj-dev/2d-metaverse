...
| `arcade_eat.ogg`, `arcade_near.ogg`, `arcade_flap.ogg`, `arcade_best.ogg` | **Project-original** — synthesized square-wave chiptune (`scripts/curate_audio.py` → `synth_arcade`) | Issue #163 per-event arcade cues, same chiptune family as the three above: Snake's bite (high two-step up), a near-miss whip (fast descending, −13 dB), the wing flap (35 ms tick, −17 dB — it fires several times a second), and the new-personal-best fanfare (rising four-note). Project asset (no third-party source). |
| `board_place.ogg`, `board_win.ogg` | Mid knock (`2-Loop(Drums Only)` @ 97.33s), metallic hit (same stem @ 94.67s), `8-Intro` notes | Issue #163 board-table foley (`scripts/curate_audio.py` → `curate_board`), replacing the borrowed arcade chiptune these events used to play: a dry, quiet wooden tap for placing a piece, and a rising G#4→C5→G#5 pluck with a soft metallic shine for the win. Board tables are plaza furniture, not cabinets, so they sit in the recorded-foley family with the rest of the world. |
