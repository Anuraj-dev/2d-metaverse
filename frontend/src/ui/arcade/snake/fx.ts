 ...
 * the final "Game over / Play again" panel — this just plays the death sting
 * before the run is reported upward.
 */
...
export interface SnakeFx {
  ...
  animating: boolean;
  /** Set once the animation finishes so the host can fire onGameOver once. */
  settled: boolean;
}
 ...
 * settles so the host can call onGameOver exactly once.
 */
