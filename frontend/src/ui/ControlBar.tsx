...
import { mediaFailureText } from "../game/controlBar";
 ...
 * emits a bus event for the sound mixer's blip, and uses its icon/pressed state as
 * the success feedback. Only failures produce an announced message. The
 * screen-share button (PRD 23) is
...
export default function ControlBar() {
  ...
  // announce the bounded reason. Successful toggles need no duplicate toast.
  const toggleDevice = (
    device: "mic" | "cam",
    on: boolean,
  ...
  const onMic = () => toggleDevice("mic", !prefs.micOn, setMic);
  const onCam = () => toggleDevice("cam", !prefs.camOn, setCam);
  const onShare = () => {
    const on = !sharing;
    setScreenShare(on);
  ...
  return (
    <div className="control-bar" role="toolbar" aria-label="Media controls" data-dialog-keep-live>
      <button
        type="button"
      ...
      <button
        ...
        data-tooltip={inMeeting ? "Share screen" : "Screen share needs a meeting"}
      >
        {sharing ? (
