...
- **Enforced pattern for all gameplay PRDs:** game logic lives in pure modules under `frontend/src/game/*.ts` (and `media/mediaLogic.ts`) — plain values in, plain values out, **no Phaser / net / DOM imports** — each landing with its vitest file in the same commit. `WorldScene.ts` stays thin glue: asset/sprite/tilemap setup, Phaser+net event wiring, and per-frame orchestration that **delegates every decision** to those modules. "Logic in the scene" (movement math, zone hit-tests, sit/stand rules, interpolation, media routing) is a review smell — extract it. See `frontend/README.md` → *Scene-as-glue + pure modules* for the module map.
- Existing pure modules to extend rather than duplicate: `movement`, `zones`, `seatDoor`, `interpolation`, `proximity`, `interactables`, `maps` (game) and `mediaLogic` (media — room names, track routing, proximity volumes). The LiveKit split is transport (`livekit.ts`, thin, untested beyond types) vs logic (`mediaLogic.ts`, pure, tested).
